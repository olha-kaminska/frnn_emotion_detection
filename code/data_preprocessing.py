...
def get_class(row):
    ...
    Input: string from 'Intensity Class' column of original emotion data, example: '1: low amount of anger can be inferred'
...
def transform_data(file_name):
    ...
    Output: Pandas DataFrame with columns: index, 'ID' (as string), 'Tweet' (original tweet), 'Cleaned_tweet' (preprocessed tweet), 'Cleaned_tweet_wt_stopwords' (preprocessed tweets after stop-words removing), 'Class' (as integer)	
    '''
    train_data = pd.read_csv(file_name, sep='\t')
    # Clean the text
    train_data["Cleaned_tweet"] = train_data["Tweet"].apply(lambda x: tweet_cleaning(x))
    #Filter stop-words
    train_data['Cleaned_tweet_wt_stopwords'] = train_data['Cleaned_tweet'].apply(lambda x: ' '.join([i for i in x.split(' ') if i not in stop_words]))
    # Obtain clean class
