...
def weights_sum_test(conf_scores, alpha):    
    ...
    conf_scores_T_rescale = [[(conf_scores_T[k][i]-0.5)/(alpha) for i in range(len(conf_scores_T[k]))] for k in range(4)]
    conf_scores_T_rescale_sum = [sum(conf_scores_T_rescale[k]) for k in range(4)]
    res = [np.exp(conf_scores_T_rescale_sum[k])/sum([np.exp(conf_scores_T_rescale_sum[k]) for k in range(4)]) for k in range(4)]
    return res
...
def test_ensemble_confscores(train_data, y, test_data, vector_names, NNeighbours, lower, upper, alpha):
    ...
    rescaled_conf_scores = np.array([weights_sum_test(conf_scores_all[:, k, :], alpha) for k in range(len(conf_scores_all[0]))])
